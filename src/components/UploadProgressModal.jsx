...
export default function UploadProgressModal({
  ...
  return (
    <ModalShell
      ...
      title={isJa ? "ファイルをアップロード中..." : "Uploading Files..."}
      subtitle={isJa ? "サーバーにファイルをアップロードしています。しばらくお待ちください。" : "Please wait while your files are being uploaded to the server."}
      maxWidth="max-w-md"
      overlayOpacity="60"
      align="center"
    >
      <div className="px-6 py-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-semibold text-[var(--text-primary)]">{isJa ? "アップロード進捗" : "Upload Progress"}</span>
        ...
        <p className="mt-3 text-xs text-[var(--text-secondary)] text-center">
          {progressPercent === 100 ? (isJa ? "完了処理中..." : "Finishing up...") : (isJa ? `アップロード中: ${progressPercent}%` : `Uploading: ${progressPercent}%`)}
        </p>
      </div>
    </ModalShell>
  );
