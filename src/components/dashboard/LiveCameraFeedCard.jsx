...
export default function LiveCameraFeedCard({ onOpenModal, onAskAI, isHighlighted }) {
  ...
  return (
    ...
    >
      ...
      {/* ── Video Player Area ── */}
      <div className="relative aspect-video w-full rounded-[6px] overflow-hidden bg-slate-900 border border-[var(--border)] flex items-center justify-center">
        <video
          ref={videoRef}
        ...
        {/* Fallback / Offline / Stream preview overlay */}
        {streamError && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center bg-slate-950 text-slate-300">
            ...
            <button
              ...
              className="mt-3 px-3 py-1.5 text-xs font-semibold rounded-[6px] bg-[var(--freya-blue)] text-white hover:bg-[var(--freya-blue-hover)] transition-colors"
            >
              Launch Diagnostics Stream
            </button>
          </div>
        )}
