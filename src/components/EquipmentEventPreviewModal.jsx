...
export default function EquipmentEventPreviewModal({ event, onClose, onEdit, onExpand }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-12 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div 
        className="w-full max-w-4xl max-h-full flex flex-col bg-[var(--surface-raised)] rounded-[12px] shadow-2xl border border-[var(--border)] overflow-hidden font-body"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)] bg-[var(--surface-raised)] shrink-0">
          <div className="flex items-center gap-3">
            <span className={`inline-flex items-center justify-center w-8 h-8 rounded-[6px] ${
            ...
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)] mb-0.5">
                {event.date} • {event["工場"]} / {event.equipmentName}
              </p>
              <h3 className="text-base font-semibold text-[var(--text-primary)] line-clamp-1">{title || t("equipmentEventFallback")}</h3>
            </div>
          </div>
          <div className="flex gap-2 items-center">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-[6px] border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] text-xs font-semibold hover:bg-[var(--surface-hover)] transition-colors"
            >
              {t("fullView")}
        ...
        {/* Scrollable Content */}
        <div className="p-6 overflow-y-auto space-y-8 bg-[var(--surface)]">
        ...
        {/* Actions Footer */}
        <div className="px-6 py-4 bg-[var(--surface-subtle)] border-t border-[var(--border)] flex items-center justify-between shrink-0">
          <button
            type="button"
            onClick={() => onEdit(event)}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-[6px] border border-[var(--border)] bg-[var(--surface)] text-xs font-semibold text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition-colors shadow-2xs"
          >
            <span className="material-symbols-outlined text-[var(--text-muted)]" style={{ fontSize: 16 }}>edit</span>
            {t("edit")}
          </button>
          ...
          <button
            type="button"
            onClick={() => onExpand(event)}
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-[6px] bg-[var(--freya-blue)] text-white text-xs font-semibold shadow-xs hover:bg-[var(--freya-blue-hover)] transition-colors"
          >
            {t("openFullTimeline")}
