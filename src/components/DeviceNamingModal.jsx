  ...
  async function handleFilesSelected(e) {
    ...
    results.forEach((r) => {
      if (r.status === "fulfilled" && r.value?.imageURL) {
        ...
        errors.push(r.reason?.message || t("uploadFailedLabel"));
      }
    });
    ...
    >
      ...
      >
        ...
        <div className="sticky top-0 z-10 rounded-t-2xl px-6 py-5 flex items-center justify-between border-b border-separator/40 bg-surface/90 backdrop-blur-md">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-outline">{t("iotDeviceLabel")}</p>
            <h2 className="mt-1 text-xl font-semibold text-on-surface">{t("renameDeviceTitle")}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div className="rounded-2xl border border-outline-variant/15 bg-surface-container-low px-4 py-3">
            <div className="flex items-center gap-2 text-outline mb-2">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-[0.18em]">{t("displayNameLabel")}</span>
            </div>
            <input
              type="text"
              value={name}
          ...
          <div className="rounded-2xl border border-outline-variant/15 bg-surface-container-low px-4 py-3">
            <div className="flex items-center gap-2 text-outline mb-2">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-[0.18em]">{t("temperatureOffsetLabel")}</span>
            </div>
            <input
              type="number"
              step="0.1"
          ...
          <div className="rounded-2xl border border-outline-variant/15 bg-surface-container-low px-4 py-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2 text-outline">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-[0.18em]">{t("devicePhotosLabel")}</span>
              </div>
              <span className="text-[10px] text-outline">{t("photosCountLabel", { count: imageURLs.length, plural: imageURLs.length !== 1 ? "s" : "" })}</span>
            </div>
            ...
            {uploadError && (
              <div className="rounded-2xl border border-error/20 bg-error/10 px-4 py-4 flex gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-on-surface">{t("uploadFailedLabel")}</p>
                ...
                {imageURLs.map((url, idx) => (
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black/50 px-2 py-1">
                      <p className="text-[10px] text-white/80">{t("photoNumberLabel", { n: idx + 1 })}</p>
                    </div>
                  </div>
                ))}
            ...
            {imageURLs.length === 0 && !isUploading && (
              <div className="rounded-xl border border-outline-variant/15 bg-surface-container/50 flex flex-col items-center justify-center gap-2 py-6 mb-3">
                ...
                <p className="text-xs text-outline">{t("noPhotosYetMessage")}</p>
              </div>
            )}
            ...
            {isUploading && imageURLs.length === 0 && (
              <div className="rounded-xl border border-outline-variant/15 bg-surface-container/50 flex flex-col items-center justify-center gap-2 py-6 mb-3">
                ...
                <p className="text-xs text-outline">{t("uploadingPhotosLabel", { count: uploadingCount, plural: uploadingCount !== 1 ? "s" : "" })}</p>
              </div>
            )}
            ...
            >
              ...
              {isUploading ? t("uploadingPhotosLabel", { count: uploadingCount, plural: uploadingCount !== 1 ? "s" : "" }) : t("addPhotosLabel")}
            </button>
          </div>
        ...
        <div className="border-t border-outline-variant/20 px-6 py-4 bg-surface-container-low/50 flex items-center justify-end gap-3">
          ...
          >
            {saving ? t("savingLabel") : t("saveChangesLabel")}
          </button>
        </div>
      </div>
    </div>
