        ...
        >
          <div className="flex items-start justify-between gap-4 border-b border-separator/40 px-5 py-4 sm:px-6 shrink-0">
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary">{isJa ? "プレビュー" : "Preview"}</p>
              <h3 className="mt-1 truncate text-lg font-semibold text-white">{name || (isJa ? "添付ファイル" : "Attached File")}</h3>
              <p className="mt-1 text-sm leading-6 text-white/65">{isJa ? "外側をクリックするか、Escキーを押して閉じます。" : "Click outside or press Escape to close."}</p>
            </div>
            <IconButton icon="close" onClick={onClose} variant="light" size="xl" ariaLabel={isJa ? "ダイアログを閉じる" : "Close dialog"} />
          </div>
          ...
          <div className="flex flex-1 items-center justify-center bg-black/25 p-4 sm:p-6 w-full min-h-0 overflow-hidden">
            {isVideoUrl(fileURL) ? (
              ...
              <iframe 
                ...
                title={name || (isJa ? "PDFプレビュー" : "PDF Preview")} 
              />
            ) : (
              <img
                src={fileURL}
                alt={name || (isJa ? "プレビュー" : "Preview")}
                className="max-h-full max-w-full rounded-[24px] object-contain shadow-2xl"
              />
            )}
          </div>
