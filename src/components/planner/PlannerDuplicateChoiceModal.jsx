...
export default function PlannerDuplicateChoiceModal({
  ...
  return (
    <PlannerModalShell
      open={open}
      title={isJa ? "重複する目標が見つかりました" : "Duplicate Goal Found"}
      subtitle={isJa ? "この製品および日付の目標は既に存在します。新しい数量の反映方法を選択してください。" : "A goal for this product and date already exists. Choose how the new quantity should be applied."}
      onClose={onClose}
    ...
    >
      <div className="space-y-3">
        <div className="rounded-[6px] border border-[var(--border)] bg-[var(--surface-subtle)] p-3 text-xs text-[var(--text-muted)]">
          ...
          <div className="mt-1">
            {isJa ? `既存: ${Number(existingGoal?.targetQuantity || 0)} 個` : `Existing: ${Number(existingGoal?.targetQuantity || 0)} pcs`}
          </div>
          <div className="mt-0.5">
            {isJa ? `今回: ${Number(pendingGoal?.targetQuantity || 0)} 個` : `Incoming: ${Number(pendingGoal?.targetQuantity || 0)} pcs`}
          </div>
        </div>
        ...
        <div className="grid gap-2.5">
          ...
          >
            <div className="text-xs font-semibold text-[var(--text-primary)]">{isJa ? "上書き" : "Overwrite"}</div>
            <div className="mt-0.5 text-xs text-[var(--text-muted)]">
              {isJa ? "既存の目標を新しい数量で置き換えます。" : "Replace the existing goal with the new quantity."}
            </div>
          </button>
          ...
          >
            <div className="text-xs font-semibold text-[var(--text-primary)]">{isJa ? "加算" : "Add"}</div>
            <div className="mt-0.5 text-xs text-[var(--text-muted)]">
              {isJa ? "既存の目標数量に今回の数量を加算します。" : "Increase the existing goal by the incoming quantity."}
            </div>
          </button>
          ...
          >
            <div className="text-xs font-semibold text-[var(--text-primary)]">{isJa ? "キャンセル" : "Cancel"}</div>
            <div className="mt-0.5 text-xs text-[var(--text-muted)]">
              {isJa ? "既存の目標を変更せずそのまま保持します。" : "Keep the existing goal unchanged."}
            </div>
          </button>
        </div>
      </div>
