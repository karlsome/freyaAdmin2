        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="planner-data-text inline-flex items-center gap-1 rounded-full border border-outline-variant/20 bg-surface-container px-3 py-1.5 font-semibold text-on-surface transition hover:bg-surface-container-high"
        ...
        {!collapsed ? (
          <div className="flex w-full flex-col gap-3 lg:w-auto lg:min-w-[360px]">
            ...
            <button
              ...
              className="rounded-2xl border border-error/20 bg-error/5 px-4 py-2 text-xs font-semibold text-error transition hover:bg-error/10 disabled:cursor-not-allowed disabled:opacity-40"
            >
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {collapsedSummary.map((item) => (
            <div key={item.label} className="rounded-full border border-outline-variant/15 bg-surface-container-low px-3 py-2">
              ...
              <div className="planner-data-text mt-1 font-semibold text-on-surface">{item.value}</div>
            </div>
          ))}
        </div>
      ) : null}
              ...
              return (
                <section key={equipment} className="rounded-3xl border border-outline-variant/15 bg-surface-container-low p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      <h4 className="planner-data-text font-semibold text-on-surface">{equipment}</h4>
                      ...
                      <div key={item._scheduleId} className="planner-data-text flex items-center justify-between gap-3 rounded-2xl bg-surface px-4 py-3">
                        <div className="min-w-0 flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="truncate font-semibold text-on-surface">{item.背番号 || item.品番 || "Product"}</div>
                        ...
                        <div className="flex items-center gap-4 text-right text-on-surface-variant">
                          <div>
                            <div className="font-semibold text-on-surface">{item.quantity} pcs</div>
