...
export default function MaterialPDFConflictModal({ open, conflicts, onClose, onConfirm }) {
  ...
  useEffect(() => {
    ...
    conflicts.existing.forEach((item) => {
      defaults[item.図番] = Array.isArray(item.pdfs) && item.pdfs.length > 1 ? "all" : "overwrite";
    });
  ...
  const newMaterials = Array.isArray(conflicts.newMaterials) ? conflicts.newMaterials : [];
          ...
          <div className="max-h-[55vh] space-y-4 overflow-y-auto px-6 py-5 scrollbar-hide">
            <div className="space-y-3">
              {existing.map((item) => {
                ...
                const selectValue = resolutions[item.図番] || (pdfCount > 1 ? "all" : "overwrite");

                return (
                  <div key={item.図番} className="rounded-2xl border border-outline-variant/15 bg-surface-container-low px-4 py-4">
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div>
                        <div className="text-sm font-semibold text-on-surface">{item.図番}</div>
